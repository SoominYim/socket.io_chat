    ...
    mutations: {
        ...
        setChats(state, { payload, getters, commit }) {
            ...
            for (var i = 0; i < payload.length; i++) {
                ...
                for (var j = 0; j < payload[j].DAILY_CHATS.length; j++) {
                    ...
                    if (
                        ...
                        // 채팅 (DB에서 가져온 채팅이 아닌 채팅창에서 적은) 이 존재해야하며
                    ...
                    if (
                        //첫번째 글이거나,
                        (payload[i],
                        DAILY_CHATS[j].USER_ID != getters.getUserId &&
                            (j == 0 ||
        ...
        // 채팅방목록에 있는 값과 비교,
        // 만약 있다면 현재 접속해 있는 상태일 것이므로, IM_ON을 true 로 바꿔준다.
        ...
        addDeleteChatList(state, payload) {
    ...
    actions: {
        async chats({ getters, dispatch, commit }, payload) {
            try {
                ...
                // 그리고 유저아이디와 방 번호를 post로 보내서 서버에 보낸다
        ...
        async chatRooms({ commit, getters, state }) {
            try {
                ...
                //그 배열을 세팅해준다
                await commit("setAreUouInAppList", userIds);
                // 챗룸을 필요한 내용을 추가하여
        ...
        async uploadImgToS3({ getters }, { formData, roomId }) {
            try {
                ...
                try {
                    ...
                    console.log("받아온 url을 db로 보낼때 생기는 오류");
                    console.log(e);
                }
            } catch (e) {
                console.log("s3에 업로들 할때 오류");
                console.log(e);
            }
        },
        async uploadVideoToS3({ getters }, { formData, roomId }) {
            ...
            try {
                ...
                const res = await axios.post(url, romData, config);
                let data = {
                    params: JSON.stringify([
                        {
                            USER_ID: getters.getUserId,
