<template lang="">
    <div class="only-chat-image">
        <div :class="{ 'len1': returnLength === 1, 'len2': returnLength === 2, 'len3': returnLength === 3, 'len4': returnLength === 4, 'len5': returnLength >= 5 }">
            <img v-for="(i, idx) in parsedImages" :key="idx" :src="i" @click="showPicture(i)" />
        </div>
        <v-dialog :value="overlayValue" @click:outside="resetPicture">
            <img  :src="nowTargetPicture" style="height:100%">
        </v-dialog>
    </div>
</template>
<script>
