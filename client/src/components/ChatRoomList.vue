<template lang="">
    <div>
        <div v-if="getErrorState">
            <h3 style="text-align : center; margin-bottom : 30px;">서버와의 연결이 불안정합니다.</h3>
            <img src="https://chium.s3.ap-northeast-2.amazonaws.com/KakaoTalk_20220506_110153004.png" alt="img" style="width:100%;">
        </div>
        <div class="msg-box" v-for="room in getChatRooms" v-else :key="room.ROOM_ID" @click="toRoom(room)">
            <div class="img-info-box">
                <div class="img-box">
                    <div class="im-on" v-if="room.IM_ON"></div>
                    <img alt="img" :src="room.AVATAR_PATH">
                </div>
                <div class="info-box">
            ...
            <div class="time-box">
                ...
                <div class="is-unread" v-fi="room.UNREAD != 0">
                    {{room.UNREAD}}
                </div>
                <div v-else class="no-unread"></div>
            </div>
...
export default {
    ...
    async created() {
        ...
        // 각 채팅방 아이디 -1 로 소켓 방에 입장시킴
        this.getChatRooms.map(v => {
            this.$enterRoom(`${v.ROOM_ID}-1`)
        })
        // 각 채팅방 목록을 실시간으로 업데이트 해주기 위한 용도
        this.$reloadChatRooms.map(() => {
            this.chatRooms()
        })
        // 메시지가 삭제 되었을 때 리프레쉬 되도록 하는 용도
        // 나중에 마지막 메시지가 삭제되었을 때 바뀌도록 수정하는게 좋음
        this.$deleteChatReloadRoomList(() => {
            this.chatRooms()
        })
    },
    destroyed() {
        ...
        // 방을 삭제하는 기능을 넣을 경우 그때 가서 이 부분을 한번 더 생각해봐야 한다.
        this.getChatRooms.map(v => {
