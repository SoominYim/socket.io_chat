<template lang="">
    <div>
        <div v-for="(chat, idx) in getChats" :key="idx">
            <div v-if="getUserId == chat.USER_ID" class="my-msg-created-and-isread">
                <!-- 삭제 보류 -->
                <!-- <input
          v-if="chat.DELETED == 0"
          type="checkbox"
        ...
        > -->
                <div v-if="chat.IS_READ != ''" class="my-msg-isread" />
                <div v-if="chat.CREATED_AT != ''" class="my-msg-created">
                    {{ chat.CREATED_AT }}
                </div>
                <div v-if="chat.MEDIA == 0" class="my-msg">
                    <span>
                        {{ chat.MESSAGE }}
                    </span>
                </div>
                ...
                <div v-else-if="chat.MEDIA == 2">
                    <video style="width: 300px" :src="chat.MESSAGE" alt="" controls />
                </div>
            </div>
            <div v-else-if="chat.USER_ID == 'system'" class="system-msg-box">
            ...
            <div v-else>
                <div v-if="chat.FIRST" class="opponent-first-msg-box">
                    <img :src="getOpponentAvatar" class="opponent-first-img" />
                    <div class="opponent-first-info-box">
                        <p class="opponent-site-name">
                            {{ getOpponentSiteName }}
                        </p>
                        <div class="opponent-msg-and-created">
                            <div v-if="chat.MEDIA == 0" class="opponent-msg">
                                <span>
                                    {{ chat.MESSAGE }}
                                </span>
                            </div>
...
<script>
import ChatImages from "@/components/ChatImages.vue"
...
export default {
    ...
    methods: {
        ...
        selectedToDelete(chatId) {
            if (index != -1) {
                const index = this.getDelChatList.splice(index, 1)
            } else {
        ...
        showSystemDay(day) {
            ...
            return `${day.slice(0, 4)}sus ${day.slice(5, 7)}월 ${day.slice(8, 10)}일 ${dayOfWeek}`
        }
    }
};
</script>
